...
def rule_supplier_concentration(db, settings) -> list:
    ...
    per_sup = (
        db.query(Claim.npi, Claim.vendor_id, Claim.vendor_name, func.count(Claim.id))
        .group_by(Claim.npi, Claim.vendor_id, Claim.vendor_name).all()
    )
    top = {}   # npi -> (count, vendor_id, vendor_name)
    ...
    for npi, (cnt, sid, sname) in top.items():
        ...
        if ratio >= CONCENTRATION_THRESHOLD:
            recent = (
                db.query(Claim.id)
                .filter(Claim.npi == npi, Claim.vendor_id == sid)
