...
function Ring({ pct, color }) {
  return (
    <svg viewBox="0 0 40 40" className="w-[34px] h-[34px] shrink-0">
      <circle cx="20" cy="20" r="16" fill="none" stroke="var(--color-bg-soft)" strokeWidth="5" />
...
export default function PlanHome({ setActiveScreen, onOpenNpi, onOpenSupplier, onOpenActivityFeed }) {
  ...
  const TILE_GRADIENTS = [
    ...
    'linear-gradient(135deg,var(--color-primary-tint),var(--color-primary))',
    'linear-gradient(135deg,#4E93B8,#2E6B8F)',
  ]

  const totalNpis = riskDist?.total || 0
  // Four bands now, straight from the shared definition — the ring segments, their
  // colors and the example names on hover all classify a score the same way the
  // backend's risk-distribution counts do.
  const ringRows = RISK_BANDS.map((b) => ({
    name: `${b.label} risk`,
    color: b.color,
    count: riskDist?.[b.id] || 0,
    band: b.id,
    examples: npis.filter((n) => riskBand(n.score).id === b.id)
  ...
  return (
    <div className="w-full">
      ...
      {/* The palette's page gradient. This portal's page is light and its dark
          surface is the hero band, so the gradient lands here. */}
      <div className="relative overflow-hidden px-4 sm:px-7 pt-6 pb-14" style={{ background: 'var(--gradient-hero)' }}>
        ...
        <div className="flex items-start justify-between gap-3 flex-wrap">
          <div>
            ...
            <p className="text-[12.5px] mt-1" style={{ color: 'var(--color-text-on-primary)' }}>What actually needs your attention right now</p>
          </div>
          <span className="text-[12px]" style={{ color: 'var(--color-text-on-primary)' }}>
      ...
      <div className="px-4 sm:px-7 pb-8 -mt-10 relative">
        ...
        <div className="grid grid-cols-1 lg:grid-cols-[220px_1fr_300px] gap-4 items-start">
          ...
          <div className="bg-white border border-[var(--color-border)] rounded-2xl px-4 py-5 text-center"
               style={{ boxShadow: 'var(--shadow-card)' }}>
            <button onClick={() => onOpenActivityFeed?.()} className="w-full cursor-pointer group">
              <div className="w-10 h-10 rounded-full mx-auto mb-2.5 flex items-center justify-center bg-[var(--color-bg-soft)] group-hover:bg-[#DCE6F7] transition-colors">
                <Icon name="alerts" size={19} className="text-[#35607D]" />
              </div>
              {loading
                ? <div className="h-9 w-14 mx-auto rounded-lg bg-slate-100 animate-pulse" />
                : <div className="text-display font-extrabold text-[36px] leading-none text-[var(--color-primary)]">{unread}</div>}
              <div className="text-[12px] text-[var(--color-text-body)] mt-1.5">Unread notification{unread !== 1 ? 's' : ''}</div>
            </button>
                    ...
                    <div className="absolute left-1/2 -translate-x-1/2 bottom-full mb-1.5 z-10 pointer-events-none whitespace-nowrap rounded-lg px-2.5 py-2 text-white"
                      ...
                      {r.examples.slice(0, 4).map((n) => (
                        <div key={n.npi} className="text-[10.5px] flex items-center justify-between gap-3">
                          ...
                          <span className="tabular-nums" style={{ color: 'var(--color-text-on-primary)' }}>{n.score}</span>
                        </div>
                      ))}
                      {r.count > 4 && <div className="text-[10px] mt-0.5" style={{ color: 'var(--color-text-on-primary)' }}>+{r.count - 4} more</div>}
                    </div>
                  )}
                  <button onClick={() => setActiveScreen('leaderboard', r.band)}
                          className="w-full flex items-center gap-3 cursor-pointer rounded-lg -mx-1 px-1 py-0.5 hover:bg-[var(--color-bg-soft)] transition-colors">
                    <Ring pct={totalNpis ? (r.count / totalNpis) * 100 : 0} color={r.color} />
                    <div className="min-w-0">
                      <div className="text-[12.5px] font-semibold text-[var(--color-primary)]">{r.name}</div>
                      <div className="text-[11px] text-[var(--color-text-body)]">{r.count} of {totalNpis}</div>
                    </div>
                  </button>
                </div>
              ))}
          ...
          <div className="bg-white border border-[var(--color-border)] rounded-2xl px-5 py-[18px]"
               style={{ boxShadow: 'var(--shadow-card)' }}>
            <div className="flex items-center justify-between gap-3">
              <h3 className="text-[15px] font-bold text-slate-900">Dispute &amp; activity feed</h3>
              <button onClick={() => onOpenActivityFeed?.()} className="text-[11.5px] font-semibold text-[var(--color-primary)] hover:underline cursor-pointer">
                View all
              </button>
            </div>
            {loading ? (
              ...
              // Scrolls instead of truncating at 5: the whole feed is reachable here,
              // and the fixed max-height keeps the claims-trend chart below from being
              // pushed down as activity accumulates.
              <div className="max-h-[264px] overflow-y-auto pr-1">
                {feed.map((n) => (
                  <div key={n.id} className="flex gap-3 py-2.5 items-start">
                    <div className="w-[52px] text-[11px] text-[var(--color-text-muted)] shrink-0 pt-0.5 tabular-nums">{feedTime(n.created_at)}</div>
                    <div className="w-[2px] self-stretch rounded-full shrink-0" style={{ background: FEED_RAIL[n.category] || 'var(--color-border)' }} />
                    <div className="min-w-0">
                      <div className="text-[13px] font-semibold text-[var(--color-primary)] leading-snug">{n.title}</div>
                      <div className="text-[11.5px] text-[var(--color-text-body)] mt-0.5 truncate">{n.description}</div>
                    </div>
                  </div>
                ))}
              </div>
                ...
                <div className="flex items-end gap-3.5 h-[150px] mt-1">
                  {trendData.map((t, i) => (
                    <div key={t.month} className="relative flex flex-col items-center gap-1.5 flex-1 min-w-0"
                      ...
                      {hoverTrend === i && (
                        <div className="absolute bottom-full left-1/2 -translate-x-1/2 mb-1.5 z-10 pointer-events-none whitespace-nowrap rounded-lg px-2.5 py-1.5 text-white"
                          ...
                          <div className="text-[10px] flex items-center gap-1.5 mt-0.5">
                            <span className="w-1.5 h-1.5 rounded-full inline-block" style={{ background: 'var(--color-primary)', boxShadow: 'inset 0 0 0 1px rgba(255,255,255,.5)' }} />
                          ...
                          {t.total > 0 && (
                            <div className="text-[10px] text-[var(--color-text-on-primary)] mt-0.5">{Math.round((t.flagged / t.total) * 100)}% flag rate</div>
                          )}
                        </div>
                      )}
                      <div className="flex items-end gap-[3px] h-[120px] cursor-default">
                        <div className="w-[9px] rounded-t-[4px] transition-opacity" style={{ height: Math.max(3, Math.round((t.total / trendMax) * 120)), background: i === peakIdx ? 'var(--color-primary)' : 'var(--color-border)', opacity: hoverTrend === null || hoverTrend === i ? 1 : 0.45 }} />
                      ...
                      <span className="text-[10.5px]" style={{ color: hoverTrend === i ? 'var(--color-primary)' : 'var(--color-text-body)', fontWeight: hoverTrend === i ? 700 : 400 }}>{t.month}</span>
                    </div>
                  ))}
                </div>
                <div className="flex gap-4 mt-2 text-[11px] text-[var(--color-text-body)]">
                  <span className="flex items-center gap-1.5"><span className="w-2 h-2 rounded-[2px] inline-block" style={{ background: 'var(--color-primary)' }} />Total claims</span>
          ...
          <div className="bg-white border border-[var(--color-border)] rounded-2xl px-5 py-[18px]"
            ...
            <div className="flex items-baseline gap-2.5 mt-2">
              {loading
                ...
                : <span className="text-display font-extrabold text-[26px] text-[var(--color-primary)]">{s.totalNPIs}</span>}
            </div>
            <div className="text-[11.5px] text-[var(--color-text-body)] mt-1">{(s.totalClaims || 0).toLocaleString()} claims across the network</div>
            <button onClick={() => setActiveScreen('leaderboard')}
                    className="mt-3 text-[11.5px] font-semibold text-[var(--color-primary)] hover:underline cursor-pointer">
