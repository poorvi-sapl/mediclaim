...
function KpiTile({ icon, label, value, accent = 'slate' }) {
  ...
  return (
    <div className="bg-white rounded-xl sm:rounded-2xl border border-slate-100 shadow-sm px-2.5 py-2.5 sm:px-5 sm:py-4 flex flex-col sm:flex-row sm:items-center gap-1.5 sm:gap-4 group cursor-default transition-all duration-200 hover:-translate-y-1 hover:shadow-md hover:border-slate-200">
      <div className={`w-7 h-7 sm:w-11 sm:h-11 rounded-lg sm:rounded-full flex items-center justify-center shrink-0 transition-transform duration-200 group-hover:scale-110 ${s.wrap}`}>
        <Icon name={icon} size={13} className={s.icon} />
      </div>
      <div className="min-w-0">
        <div className="text-[1.1rem] sm:text-[1.45rem] font-bold tabular-nums leading-none tracking-tight text-slate-900 truncate">{value}</div>
        <div className="text-[9px] sm:text-[11px] font-medium text-slate-400 mt-0.5 sm:mt-1 uppercase tracking-wider leading-none">{label}</div>
      </div>
    </div>
  )
}
...
export default function FlaggedSuppliers({ suppliers = [], onSelectSupplier }) {
  ...
  return (
    <div className="w-full px-4 sm:px-7 py-4 sm:py-7">
      ...
      {total === 0 ? (
        <div className="mc-card py-12 sm:py-16 text-center px-6">
        ...
        <div className="mc-card overflow-hidden">
          ...
          {/* Card header */}
          <div className="px-4 sm:px-5 py-3 sm:py-4 border-b border-slate-100 flex flex-col sm:flex-row sm:items-center gap-2 sm:gap-0">
          ...
          <div className="sm:hidden divide-y divide-slate-100">
            {suppliers.map((s) => (
              <div key={s.id} onClick={() => onSelectSupplier?.(s.name)}
                   className="px-4 py-3.5 flex items-center gap-3 cursor-pointer hover:bg-slate-50 active:bg-slate-100 transition-colors">
                <div className="w-9 h-9 rounded-xl bg-rose-50 text-rose-500 ring-1 ring-rose-100 flex items-center justify-center flex-shrink-0">
                  <Icon name="flag" size={14} />
                </div>
                <div className="flex-1 min-w-0">
                  <div className="text-[13px] font-semibold text-slate-800 truncate">{s.name}</div>
                  <div className="flex items-center gap-1.5 mt-0.5 flex-wrap">
                    <span className="text-[11px] text-slate-500 tabular-nums">{s.claimsCount} claims</span>
                    <span className="text-slate-300 text-[10px]">·</span>
                    <span className="text-[11px] text-slate-400 tabular-nums">{fmtDate(s.flaggedAt || s.firstFlagged)}</span>
                  </div>
                  {/* Mini bar */}
                  <div className="mt-1.5 h-1 rounded-full bg-slate-100 overflow-hidden">
                    <div className="h-full rounded-full bg-[#1B3A5C]/50 transition-all duration-300"
                         style={{ width: `${Math.round((s.claimsCount / maxClaims) * 100)}%` }} />
                  </div>
                </div>
                <div className="text-right flex-shrink-0 flex flex-col items-end gap-1">
                  <div className="text-[13px] font-bold text-slate-800 tabular-nums">{fmtUSD(s.totalAmount)}</div>
                    ...
                    <td className="td">
                      <div className="flex items-center gap-3 min-w-[160px]">
                        ...
                        <div className="flex-1 h-1.5 rounded-full bg-slate-100 overflow-hidden min-w-[60px]">
                          <div className="h-full rounded-full bg-[#1B3A5C]/60 transition-all duration-300"
                               style={{ width: `${Math.round((s.claimsCount / maxClaims) * 100)}%` }} />
                        </div>
                      </div>
                    </td>
      ...
      {/* Info banner */}
      <div className="mt-4 sm:mt-5 mc-card px-4 sm:px-5 py-3.5 sm:py-4 flex items-start gap-3 sm:gap-3.5 border-[#1B3A5C]/10">
